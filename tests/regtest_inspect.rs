...
use cosigner::config::{ChainNetwork, KeySpec, KeysConfig, WalletConfig};
...
fn regtest_wallet_config() -> WalletConfig {
    let path = "48h/1h/0h/2h";
    WalletConfig {
        network: ChainNetwork::Regtest,
        i_understand_this_is_mainnet: false,
        timelock_blocks: TIMELOCK_BLOCKS,
        keys: KeysConfig {
            satochip: key_spec(0xA1, path),
            mobile: key_spec(0xA2, path),
            server: key_spec(0xA3, path),
        },
        bitcoind: None,
        server: None,
        policy: None,
